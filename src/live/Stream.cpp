 ...
 * @Date: 2025-07-29 13:07:47
 * @LastEditors: jiangshan yaoranyaoran2015@outlook.com
 * @LastEditTime: 2025-07-29 13:12:25
 * @FilePath: /liveStream-study/src/live/Stream.cpp
 * @Description: 
 * @
...
{
    stream_time_.store(TTime::getCurrentTimeInMS());
...
bool Stream::Timeout()
{
    ...
    // 应该是在每次都有流数据的时候同步更新
    auto delta = TTime::getCurrentTimeInMS() - stream_time_.load();
    if (delta > 20 * 1000)
    {
        return true;
    }
...
{
    ready_ = ready;
    ready_time_ = TTime::getCurrentTimeInMS();
}
...
void Stream::AddPacket(PacketPtr &&packet)
{
    ...
    // 里面是要访问has_video_等多线程变量的，所以加锁
    {
        ...
        // ProcessHls(packet); // 拿到flv数据，处理成mpegts格式给hls使用
    ...
    {
        data_comming_time_ = TTime::getCurrentTimeInMS();
    }
...
void Stream::GetFrames(const PlayerUserPtr &user)
{
    ...
    if (user->out_index_ >= 0)
    {
        ...
        if (user->out_index_ < min_idx || (gop_mgr_.LastestTimeStamp() - user->out_frame_timestamp_) > 2 * content_lantency)
        {
...
// /// @brief 在stream中处理mpegts，组装成hls切片
// /// @param packet
