...
{
    int32_t ret = 0;
    if (state_ == kRtmpHandShake || state_ == kRtmpWatingDone) // 正在握手状态，开始握手
    {
...
void RtmpContext::OnWriteComplete()
{
    ...
    // {
    //     // 这里有bug，忽略了客户端延时接收到S2的情况
    //     // 中途完成握手后，缺少对S2消息的处理，会使得后续的RTMP命令消息解析失败
    //     // 修改了RtmpContext::Parse函数，使得握手完成后，可以顺利接收消息
    //     // 这里没有用了。
    //     state_ = kRtmpMessage;
    //     // 可能是在中途完成了握手
    //     if (is_client_)
...
{
    uint8_t fmt;                                // Basic Header格式
    uint32_t csid;                              // Chunk Stream ID
    uint32_t msg_len = 0;                       // 消息长度
    uint32_t msg_sid = 0;                       // 消息流ID
    uint8_t msg_type = 0;                       // 消息类型
    uint32_t total_bytes = buf.ReadableBytes(); // 缓冲区中可读的字节数
    int32_t parsed = 0;                         // 已解析的字节数

    in_bytes_ += (buf.ReadableBytes() - last_left_); // 更新接收的字节数
    SendBytesRecv();                                 // 发送接收字节数的统计信息

    while (total_bytes > 1) // 循环解析，直到缓冲区中数据不足
    {
        const char *pos = buf.Peek(); // 获取缓冲区当前可读数据的指针
        parsed = 0;                   // 重置已解析的字节数

        // 解析Basic Header
        fmt = (*pos >> 6) & 0x03; // 获取格式
        csid = *pos & 0x3F;       // 获取Chunk Stream ID
        parsed++;
        ...
        // 根据格式解析消息长度
        msg_len = prev->msg_len; // 初始化消息长度为之前的消息长度
        if (fmt == kRtmpFmt0 || fmt == kRtmpFmt1)
        {
            // 如果格式为kRtmpFmt0或kRtmpFmt1，从缓冲区中读取新的24位消息长度
            msg_len = BytesReader::ReadUint24T((pos + parsed) + 3);
        }
        else if (msg_len == 0)
        {
            // 如果格式不是kRtmpFmt0或kRtmpFmt1，并且消息长度为0，使用默认的chunk大小作为消息长度
        ...
        buf.Retrieve(parsed);  // 移除已解析的数据
        total_bytes -= parsed; // 更新剩余数据大小
        ...
        if (packet->Space() == 0)
        {
            ...
            MessageComplete(std::move(packet)); // 处理完成的数据包
            packet.reset();                     // 重置数据包指针
        }
    }
    return 1; // 数据不足
}
...
{
    RTMP_TRACE << "recv message type : " << data->PacketType()
               << " len : " << data->PacketSize();
...
bool liveStream::mm::RtmpContext::BuildChunk(const PacketPtr &packet, uint32_t timestamp, bool fmt0)
{
    RtmpMsgHeaderPtr header = packet->Ext<RtmpMsgHeader>();
    if (header)
    {
        ...
        // 1. 不指定fm0
        // 2. 没有前一个，第一个肯定是fmt0
        // 3. 当前时间戳大于前一个的就不是fmt0,
        // 4.当前的msg_sid和前一个一样，肯定不是fmt0
        // 此变量目的是判断是否使用delta时间戳，
        // 如果为真，则使用delta时间戳（不是fmt0的chunk header），
        // 如果为假，则使用timestamp，（只有在fmt0的情况下才会使用timestamp）
...
void liveStream::mm::RtmpContext::Send()
{
    ...
    connection_->send(sending_bufs_); // 发送数据
}

bool liveStream::mm::RtmpContext::Ready() const
{
    return !sending_;
}

void liveStream::mm::RtmpContext::SendSetChunkSize()
{
    ...
    RTMP_DEBUG << "send chunk size:" << out_chunk_size_ << " to host:" << connection_->getPeerAddress().toIpPort();
    // RTMP_TRACE << "send chunk size:" << out_chunk_size_ << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet)); // 加入需要build包的数据队列中，等待build，然后send
}

void liveStream::mm::RtmpContext::SendAckWindowSize()
{
    PacketPtr packet = Packet::NewPacket(64);                    // 创建消息包
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>(); // 创建消息头
    if (header)
    ...
    RTMP_DEBUG << "send ack size:" << ack_size_ << " to host:" << connection_->getPeerAddress().toIpPort();
    // RTMP_TRACE << "send ack size:" << ack_size_ << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet)); // 加入需要build包的数据队列中，等待build，然后send
}

void liveStream::mm::RtmpContext::SendSetPeerBandwidth()
{
    PacketPtr packet = Packet::NewPacket(64);                    // 创建消息包
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>(); // 创建消息头
    if (header)
    ...
    *body++ = 0x02; // 设置带宽要多加一个，限制类型，设置为2是动态的
    ...
    // RTMP_TRACE << "send band width:" << ack_size_ << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet)); // 加入需要build包的数据队列中，等待build，然后send
}

void liveStream::mm::RtmpContext::SendBytesRecv()
{
...
void liveStream::mm::RtmpContext::SendUserCtrlMessage(short nType, uint32_t value1, uint32_t value2)
{
    PacketPtr packet = Packet::NewPacket(64);                    // 创建消息包
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>(); // 创建消息头
    if (header)
    ...
    RTMP_DEBUG << "send control type:" << nType
               << " value1:" << value1
               << " value2:" << value2
               << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet)); // 加入需要build包的数据队列中，等待build，然后send
}

void liveStream::mm::RtmpContext::HandleChunkSize(PacketPtr &packet)
{
    ...
    else
    {
        RTMP_ERROR << "invalid chunk size packet msg_len:" << packet->PacketSize()
                   << " host :" << connection_->getPeerAddress().toIpPort();
    }
}

void liveStream::mm::RtmpContext::HandleAckWindowSize(PacketPtr &packet)
{
...
void liveStream::mm::RtmpContext::HandleUserMessage(PacketPtr &packet)
{
    ...
    RTMP_TRACE << "recv user control type:" << value << " host :" << connection_->getPeerAddress().toIpPort();
    switch (type)
    {
    case kRtmpEventTypeStreamBegin: // 流开始
    {
        RTMP_TRACE << "recv stream begin value:" << value << " host :" << connection_->getPeerAddress().toIpPort();
        break;
    }
    case kRtmpEventTypeStreamEOF: // 流结束
    {
        RTMP_TRACE << "recv stream eof value:" << value << " host :" << connection_->getPeerAddress().toIpPort();
        break;
    }
    case kRtmpEventTypeStreamDry: // 流变为空（无数据）
    {
        RTMP_TRACE << "recv stream dry value" << value << " host:" << connection_->getPeerAddress().toIpPort();
        break;
    }
    case kRtmpEventTypeSetBufferLength: // 设置缓冲长度
    {
        RTMP_TRACE << "recv set buffer length value" << value << " host:" << connection_->getPeerAddress().toIpPort();
        if (msg_len < 10)
        {
            RTMP_ERROR << "invalid user control packet msg_len:" << packet->PacketSize()
                       << " host:" << connection_->getPeerAddress().toIpPort();
    ...
    {
        RTMP_TRACE << "recv stream recoded value" << value << " host:" << connection_->getPeerAddress().toIpPort();
        break;
    }
    case kRtmpEventTypePingRequest: // Ping 请求
    {
        RTMP_TRACE << "recv ping request value" << value << " host:" << connection_->getPeerAddress().toIpPort();
        SendUserCtrlMessage(kRtmpEventTypePingResponse, value, 0);
        break;
    }
    case kRtmpEventTypePingResponse: // Ping 响应
    {
        RTMP_TRACE << "recv ping response value" << value << " host:" << connection_->getPeerAddress().toIpPort();
        break;
    }
    default:
        break;
...
void liveStream::mm::RtmpContext::HandleAmfCommand(PacketPtr &data, bool amf3)
{
    RTMP_TRACE << "amf message len:" << data->PacketSize() << " host:" << connection_->getPeerAddress().toIpPort();
    ...
    {
        RTMP_ERROR << "amf decode failed. host:" << connection_->getPeerAddress().toIpPort();
        return;
    }
    // obj.Dump();
    ...
    RTMP_TRACE << "amf command:" << method << " host:" << connection_->getPeerAddress().toIpPort();

    auto iter = commands_.find(method);
    if (iter == commands_.end()) // 不支持这个命令
    {
        RTMP_TRACE << "not surpport method : " << method << " host : " << connection_->getPeerAddress().toIpPort();
        return;
    }
...
void liveStream::mm::RtmpContext::SendConnect()
{
    ...
    *p++ = 0x09;
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "send connect msg_len:" << header->msg_len << " host:" << connection_->getPeerAddress().toIpPort();

    // 加入到待编码的队列中
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::HandleConnect(AMFObject &obj)
{
    auto amf3 = false;
    ...
    *p++ = 0x09;
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "connect result msg_len:" << header->msg_len << " host:" << connection_->getPeerAddress().toIpPort();

    // 加入到待编码的队列中
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::SendCreateStream()
{
    PacketPtr packet = Packet::NewPacket(1024);
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>();
    ...
    *p++ = kAMFNull;
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "send create stream msg_len:" << header->msg_len << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::HandleCreateStream(AMFObject &obj)
{
    ...
    *p++ = kAMFNull;
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "create stream result msg_len:" << header->msg_len << " to host:" << connection_->getPeerAddress().toIpPort();
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::SendStatus(const std::string &level, const std::string &code, const std::string &description)
{
    PacketPtr packet = Packet::NewPacket(1024);
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>();
    header->cs_id = kRtmpCSIDAMFIni; // 使用AMF的初始化，固定为3
    ...
    *p++ = 0x09;
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "send status level:" << level << "code:" << code << "description:" << description << " host:" << connection_->getPeerAddress().toIpPort();

    // 加入到待编码的队列中
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::SendPlay()
{
    PacketPtr packet = Packet::NewPacket(1024);
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>();
    header->cs_id = kRtmpCSIDAMFIni; // 使用AMF的初始化，固定为3
    ...
    *p++ = kAMFNull;                      // 空值
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "send play name:" << name_ << "msg_len = " << header->msg_len << " host:" << connection_->getPeerAddress().toIpPort();

    // 加入到待编码的队列中
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::HandlePlay(AMFObject &obj)
{
    ...
    RTMP_TRACE << "recv play session_name:" << session_name_ << " param:" << param_ << " host:" << connection_->getPeerAddress().toIpPort();
...
void liveStream::mm::RtmpContext::ParseNameAndTcUrl()
{
    ...
    std::vector<std::string> list = base::StringUtils::split(tc_url_, "/");
    if (list.size() >= 4) // rmtp://ip/domain:port/app/stream
    {
        domain = list[2];
        app_ = list[3];
        if (list.size() > 4)
    ...
    {
        RTMP_ERROR << "Url is : \"rtmp://(ip:port || domain:port)/app_name/stream_name\", but got : " << tc_url_;
        exit(-1);
    }
...
void liveStream::mm::RtmpContext::SendPublish()
{
    PacketPtr packet = Packet::NewPacket(1024);
    RtmpMsgHeaderPtr header = std::make_shared<RtmpMsgHeader>();
    header->cs_id = kRtmpCSIDAMFIni; // 使用AMF的初始化，固定为3
    ...
    *p++ = kAMFNull; // 空值
    ...
    header->msg_len = p - body;
    packet->SetPacketSize(header->msg_len);
    RTMP_TRACE << "send publish name:" << name_ << "msg_len = " << header->msg_len << " host:" << connection_->getPeerAddress().toIpPort();

    // 加入到待编码的队列中
    PushOutQueue(std::move(packet));
}

void liveStream::mm::RtmpContext::HandlePublish(AMFObject &obj)
{
    auto tran_id = obj.Property(1)->Number();
    name_ = obj.Property(3)->String();
...
void liveStream::mm::RtmpContext::HandleResult(AMFObject &obj)
{
    auto id = obj.Property(1)->Number();
    RTMP_TRACE << "recv result id: " << id << " host:" << connection_->getPeerAddress().toIpPort();
    if (id == 1) // connect
    {
...
void liveStream::mm::RtmpContext::HandleError(AMFObject &obj)
{
    // 拿到描述
    const std::string &description = obj.Property(3)->Object()->Property("description")->String();
    RTMP_TRACE << "recv error description: " << description << " host:" << connection_->getPeerAddress().toIpPort();
    connection_->forceClose();
}

void liveStream::mm::RtmpContext::SetPacketType(PacketPtr &packet)
{
...
void liveStream::mm::RtmpContext::Play(const std::string &url)
{
    is_client_ = true;
    is_player_ = true;
    tc_url_ = url;
...
void liveStream::mm::RtmpContext::Publish(const std::string &url)
{
    is_client_ = true;
    is_player_ = false; // 推流
    tc_url_ = url;
...
bool liveStream::mm::RtmpContext::BuildChunk(PacketPtr &&packet, uint32_t timestamp, bool fmt0)
{
    RtmpMsgHeaderPtr header = packet->Ext<RtmpMsgHeader>();
    if (header)
    {
        ...
        // 1. 不指定fm0
        // 2.有前一个，才会进行后面的包时间计算
        // 3. 当前时间戳大于前一个的就不是fmt0,  当前的msg_sid和前一个一样，肯定不是fmt0
        bool use_delta = !fmt0 && prev && timestamp >= prev->timestamp && header->msg_sid == prev->msg_sid;
        if (!prev)
        {
            prev = std::make_shared<RtmpMsgHeader>(); // 一开始都是创建新的
